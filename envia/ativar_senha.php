...
$base_url = 'http://' . $_SERVER['HTTP_HOST'] . '/camara_mogi/webapp/';

$email = explode("ativar_senha/", $_SERVER["REQUEST_URI"]);
...
if ($stmt_email->execute()) {
    ...
    echo'<script>window.location.href = "'.$base_url.'restrita/usuarios/usuario_criado_erro";</script>';
    exit;
}
...
if ($stmt_sistema->execute()) {
    ...
    exit;
			...
			// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
			$assunto = 'Perfil de usuário ativo'; 
			...
			$mail->Body = '
                    ...
                    <body>
                        <div class="email">
                            <p><b class="destaque">Olá, '.$result_email['nome'].'. Seu perfil de usuário esta ativo para acessar o painel '.$result_sistema['sistema_site_titulo'].'.<br>
                            <a href="'.$base_url.'restrita/login/ativar_senha/'.$result_email['token'].'">clique aqui para gerar sua senha.</a></p>
            ...
            if ($enviado){
                ...
                echo'<script>window.location.href = "'.$base_url.'restrita/usuarios/email_enviado/'.$result_email['id'].'";</script>';
                exit;

            }else{

                echo'<script>window.location.href = "'.$base_url.'restrita/usuarios/email_envia_erro/'.$result_email['id'].'";</script>';
                exit;
