...
class Controller extends CI_Controller
{
	...
	public function core($pag_id = null)
	{
		...
		if (!$pag_id) {
			...
			if ($area->adicionar) {
				...
				if ($this->form_validation->run()) {
					...
					$artigo_last = $this->core_model->get_by_id('artigos', array('pag_id' => $this->session->userdata('last_id')));
					...
					for ($i = 0; $i < $total_fotos; $i++) {
						...
						$data = array(
							'foto_pag_id' => $artigo_last->pag_id,
					...
					if ($semelhantes) {
						...
						for ($i = 0; $i < $total_semelhantes; $i++) {
							...
							$semelhantes_data = array(
								'pag_id' => $artigo_last->pag_id,
								'pag_id_semelhante' => $semelhantes[$i],
							);
					...
					if ($subcategorias_id) {
						...
						for ($i = 0; $i < $qty_subcategorias; $i++) {
							...
							$data_subcategorias = array(
								'ca_id_artigo' => $artigo_last->pag_id,
					...
					redirect('restrita/' . $this->router->fetch_class() . '/view/' . $artigo_last->pag_id);
				} else {
			...
			if ($area->editar) {
				...
				if (!$artigo = $this->artigos_model->get_by_id(array('pag_id' => $pag_id))) {
					...
					if ($this->form_validation->run()) {
						...
						$log_query = $this->core_model->update('artigos', $data, array('pag_id' => $artigo->pag_id));

						$this->core_model->delete_msn('artigos_fotos', array('foto_pag_id' => $artigo->pag_id));
						...
						if ($ctas) :
							...
							$this->core_model->delete_msn('artigos_banner_cta', array('aux_pag_id' => $artigo->pag_id));

							for ($i = 0; $i < $total_ctas; $i++) {

								$cta_data = array(
									'aux_pag_id' => $artigo->pag_id,
									'aux_cta_codigo' => $ctas[$i],
								);
						...
						$this->core_model->delete_msn('aux_categoria_artigos', array('ca_id_artigo' => $artigo->pag_id));
						...
						$semelhantes = $this->input->post('artigos_semelhantes');
						$this->core_model->delete_msn('artigos_semelhantes', array('artigos_semelhantes.pag_id' => $artigo->pag_id));
						...
						$seo = $this->input->post('artigo_seo');
						$this->core_model->delete_msn('artigo_palavras_seo', array('seo_pag_id' => $artigo->pag_id));
						...
						redirect('restrita/' . $this->router->fetch_class() . '/view/' . $artigo->pag_id . '/editar');
					} else {
						...
						$data = array(
							...
							'artigo' => $artigo,
							'seo' => $this->artigos_model->get_all_artigos_seo(array('seo_pag_id' => $artigo->pag_id)),
							'fotos_artigo' => $this->core_model->get_all('artigos_fotos', array('foto_pag_id' => $artigo->pag_id)),
							'categorias_pai' => $this->artigos_model->get_all_categorias_pai_artigos(array('categorias_pai.categoria_pai_ativa' => 1)),
							'subcategorias_do_artigo' => $this->aux_artigos_categoria->get_all_categorias_do_artigo(array('ca_id_artigo' => $artigo->pag_id)),
							'banners_do_artigo' => $this->banners_cta_model->get_do_artigo(array('aux_pag_id' => $artigo->pag_id)),
							'artigos_semelhantes_do_artigo' => $this->artigos_model->get_all_artigos_semelhantes(array('artigos_semelhantes.pag_id' => $artigo->pag_id)),
						);
	...
	public function view($pag_id = null)
	{
		...
		if (!$artigo = $this->artigos_model->get_by_id(array('pag_id' => $pag_id))) {
			...
			$data = array(
				...
				'seo' => $this->artigos_model->get_all_artigos_seo(array('seo_pag_id' => $artigo->pag_id)),
				'fotos_artigo' => $this->core_model->get_all('artigos_fotos', array('foto_pag_id' => $artigo->pag_id)),
				'artigos_semelhantes_do_artigo' => $this->artigos_model->get_all_artigos_semelhantes(array('artigos_semelhantes.pag_id' => $artigo->pag_id, 'artigos.artigo_publicado' => 1)),
				'subcategorias_do_artigo' => $this->aux_artigos_categoria->get_all_categorias_do_artigo(array('ca_id_artigo' => $artigo->pag_id)),
				'banners_do_artigo' => $this->banners_cta_model->get_do_artigo(array('aux_pag_id' => $artigo->pag_id)),
			);
	...
	public function get_artigos()
	{
		...
		$pag_id = $this->input->post('pag_id');

		$artigo = $this->core_model->get_all('artigos', array('artigo_titulo' => $artigo_titulo, 'pag_id !=' => $pag_id));
	...
	public function salva_leitura()
	{
		...
		if ($artigo_leitura) {
			...
			$this->core_model->update_ajax('artigos', array('artigo_tempo_leitura' => $artigo_leitura), array('pag_id' => $pag_id));
		}
	...
	public function valida_titulo_artigo($artigo_titulo)
	{
		...
		if (!$pag_id) {
			...
			if ($this->core_model->get_by_id('artigos', array('artigo_titulo' => $artigo_titulo, 'pag_id !=' => $pag_id))) {
	...
	public function delete($pag_id = null)
	{
		...
		if ($area->excluir) {
			...
			if (!$pag_id || !$artigo = $this->artigos_model->get_by_id(array('pag_id' => $pag_id))) {
				$this->session->set_flashdata('erro', 'Artigo não encontrado');
			...
			$log_query = $this->core_model->delete('artigos', array('pag_id' => $artigo->pag_id));
			$this->core_model->delete('artigos_banner_cta', array('aux_pag_id' => $artigo->pag_id));
			$this->core_model->delete('aux_categoria_artigos', array('ca_id_artigo' => $artigo->pag_id));
	...
	public function situacao($pag_id = null)
	{
		...
		if ($area->editar) {
			...
			$this->core_model->update('artigos', $data, array('pag_id' => $artigo->pag_id));
		}
